...
// File: contracts/gsys/gsysPool.sol
...
contract GsysPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 gsysAtLastUserAction; // keep track of gsys deposited at the last user action.
    ...
    event NewVGsysContract(address VGsys);
    ...
    /**
     * @notice Constructor
     * @param _token: Gsys token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: gsys pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(gsysPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the gsys owner address or the operator address.
     */
    modifier onlyOperatorOrGsysOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or gsys owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onGsysPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.gsysAtLastUserAction;
    ...
    /**
     * @notice Unlock user gsys funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrGsysOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Gsys Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in GSYS)
    ...
    function depositOperation(
        ...
        if (VGsys != address(0)) {
            IVGsys(VGsys).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.gsysAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Gsys Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VGsys != address(0)) {
            IVGsys(VGsys).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.gsysAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.gsysAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending GSYS tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingGsys = masterchefV2.pendingGsys(gsysPoolPID, address(this));
        if (pendingGsys > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(gsysPoolPID, 0);
    ...
    /**
     * @notice Set VGsys Contract address
     * @dev Callable by the contract admin.
     */
    function setVGsysContract(address _VGsys) external onlyAdmin {
        require(_VGsys != address(0), "Cannot be zero address");
        VGsys = _VGsys;
        emit NewVGsysContract(VGsys);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Gsys Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingGsysRewards();
     ...
     * @return Returns total pending gsys rewards
     */
    function calculateTotalPendingGsysRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingGsys(gsysPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingGsysRewards()) * (1e18)) / totalShares);
    }
