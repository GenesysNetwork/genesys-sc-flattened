...
contract GsysVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 gsysAtLastUserAction; // keeps track of gsys deposited at the last user action
    ...
    /**
     * @notice Deposits funds into the Gsys Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.gsysAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Gsys Vault
     */
     ...
     */
    function calculateHarvestGsysRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGsys(0, address(this));
     ...
     */
    function calculateTotalPendingGsysRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingGsys(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the Gsys Vault
